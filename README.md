...
- **Продукты** — база продуктов с КБЖУ на 100г, поиск по имени, штрихкоды
- **OpenFoodFacts** — локальная база ~2M продуктов (SQLite + FTS5), мгновенный поиск и lookup по штрихкоду без внешних API
...
| `search_product` | Поиск по имени (локально + OpenFoodFacts), сортировка по частоте |
| `lookup_product` | Найти продукт по штрихкоду (локальная БД → OpenFoodFacts → кэш) |
| `log_meal` | Записать приём пищи (по product_id или ad-hoc) |
...
| `get_top_products` | Топ продуктов за N дней (для списков покупок, анализа привычек) |
